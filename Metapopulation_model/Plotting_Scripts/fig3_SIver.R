...
.args <- if (interactive()) c(
  ...
  file.path("Figures", "fig3_SIver.png")
...
refscen <- expression(
  movModel == 0 & (moveRate == 0 | moveRate == 0.1) & ES_Detection == 0 & vacRate == 0 &
  !(label %like% "&")
)
...
#    , scales = "free_y"
  ...
  aes(
    len/365, value,
    color = label, alpha = factor(moveRate), group = interaction(label, moveRate)
  ) + # facet_grid(moveRate ~ vacRate) +
  geom_line() +
  geom_text(
    aes(label = label, color = NULL, alpha = NULL, group = NULL),
