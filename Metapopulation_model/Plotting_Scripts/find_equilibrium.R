...
find_equilibrium <-function(pop_size,vacc_rate){
  ...
  return(data.frame(S = out[length(times),2]/pop_size,
                    I1 = out[length(times),3]/pop_size,
                    V = out[length(times),4]/pop_size,
                    R = out[length(times),5]/pop_size,
                    P = out[length(times),6]/pop_size,
                    Ir = out[length(times),7]/pop_size))
}
