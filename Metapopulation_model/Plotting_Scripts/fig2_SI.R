...
refscen <- expression(movModel == 0 & moveRate == 0.1 & ES_Detection == 0 & vacRate == 0)
...
)[eval(refscen)][, hetero := grepl("&", label) ]
...
p <- ggplot(plot.src[!is.na(value) & !is.infinite(value)]) +
  aes(len/365, value, color = label, linetype = hetero) +
  facet_grid_sc(
  ...
  geom_line() +
  geom_text(
    aes(label = label, color = NULL, linetype=NULL),
    data = function(dt) dt[, .(len = 365/10, value = (max(value)-min(value))*.90+min(value), label = c("A", "B", "C")[as.numeric(exp)]), by=exp],
    size = 8,
    show.legend = FALSE
  ) +
  geom_blank(aes(color = NULL, linetype=NULL), data = function(dt) dt[,.(len = c(0,3.5)*365, value = 0),by=exp]) +
  geom_label_repel(
  ...
  scale_linetype_manual(
    NULL,
    labels = c(`TRUE`="mixed", `FALSE`="matching"),
...
theme_trim <- list(
  ...
  geom_blank(aes(color = NULL, linetype=NULL), data = data.table(len = c(2.5, 3.5)*365, value = 0))
)
...
pinset1axis <- ggplot(
  plot.src[!is.na(value) & !is.infinite(value)][variable == "p.sc"][between(len/365, 2.5, 3.5)]
) + crossbits + geom_blank(data = function(dt) dt[,.(len = rep(len[1],2), value = range(value)),by=.(label, hetero)]) +theme(
