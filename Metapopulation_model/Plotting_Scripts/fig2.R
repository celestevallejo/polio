...
p <- ggplot(plot.src[!is.na(value) & !is.infinite(value)]) +
  ...
  geom_blank(aes(color = NULL), data = function(dt) dt[,.(len = c(0,3.5)*365, value = 0),by=exp]) +
  geom_label_repel(
...
theme_trim <- list(
  ...
  geom_blank(aes(color = NULL), data = data.table(len = c(2.5, 3.5)*365, value = 0))
)
...
pinset1axis <- ggplot(
  plot.src[!is.na(value) & !is.infinite(value)][variable == "p.sc"][between(len/365, 2.5, 3.5)]
) + crossbits + geom_blank(data = function(dt) dt[,.(len = rep(len[1],2), value = range(value)),by=label]) +theme(
