...
refscen <- expression(movModel == 0 & vacRate == 0 & moveRate == 0.1)
...
p <- ggplot(plot.src[!is.na(value) & !is.infinite(value)]) +
  aes(len/365, value, color = factor(ES_Detection), linetype = label, group = interaction(label, ES_Detection)) +
  facet_grid_sc(
  ...
  geom_line() +
  geom_text(
    aes(label = label, color = NULL, group = NULL, linetype = NULL),
    data = function(dt) dt[, .(len = 365/10, value = (max(value)-min(value))*.90+min(value), label = c("A", "B", "C")[as.numeric(exp)]), by=exp],
    size = 8,
    show.legend = FALSE
  ) +
  geom_blank(aes(color = NULL, group = NULL, linetype = NULL), data = function(dt) dt[,.(len = c(0,3.5)*365, value = 0),by=exp]) +
  # geom_label_repel(
  #   aes(label=label), data=lbls, show.legend = FALSE,
  ...
  scale_color_manual(
    "ES Obs. %", values = marker, labels = function(bs) scales::percent(as.numeric(as.character(bs))),
  ...
  theme(
    ...
    legend.position = c(.95, .95), legend.justification = c(1, 1),
...
#   ),
#   geom_blank(aes(color = NULL, linetype = NULL, group = NULL), data = data.table(len = c(2.5, 3.5)*365, value = 0))
# )
...
#   data = function(dt) dt[,.(len = rep(len[1],2), value = range(value)),by=.(ES_Detection, label)]
# ) +theme(
...
# pinset2axis <- ggplot(
#   plot.src[!is.na(value) & !is.infinite(value)][variable == "del.p"][between(len/365, 2.5, 3.5)]
# ) + crossbits + geom_blank(data = function(dt) dt[,.(len = rep(len[1],2), value = range(value)),by=.(ES_Detection, label)]) +theme(
#   axis.text.x = element_blank(),
#   axis.ticks.x = element_blank(),
#   axis.ticks.length.x = unit(0, "null"),
#   axis.title.x = element_blank(),
